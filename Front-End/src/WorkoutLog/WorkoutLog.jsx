...
const AUTH0_AUDIENCE = "https://dev-o87gtr0hl6pu381w.us.auth0.com/api/v2/"; 
...
const saveWorkout = async () => {
  if (!isAuthenticated) {
    await loginWithRedirect({
      appState: { 
        ...
        workoutData: { // Save current workout state
          name: workoutName,
  ...
  try {
    ...
    const token = await getAccessTokenSilently({
      detailedResponse: true, // Get full token response
      authorizationParams: {
        audience: AUTH0_AUDIENCE,
        scope: "write:workouts"
      },
      cacheMode: 'off' // Bypass cache
    });

    console.log("Token details:", token); // Debugging
